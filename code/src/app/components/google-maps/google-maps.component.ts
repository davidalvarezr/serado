...
export class GoogleMapsComponent implements OnInit, AfterViewInit {
    ...
    if (this.adPosition) {
      setTimeout(() => {
        ...
        if (this.currentPosition !== null) {
          ...
          this.map.setCenter(
            new maps.LatLng(this.adPosition.lat, this.adPosition.lng),
          );
          this.map.setZoom(10);
        }
      }, 10);
    } else {
      // Show that the position of the ad is not available
    }
  }

  private addMarker(latLng: LatLng): maps.Marker {
    const marker = new maps.Marker({ position: latLng, map: this.map });
  ...
  private addCurrentLocationMarker(latLng: LatLng): maps.Marker {
    const marker = new maps.Marker({
      position: latLng,
      map: this.map,
      icon: '../../assets/map/cur_loc_32_s.png',
    });
    this.markers.push(marker);
    return marker;
  }
